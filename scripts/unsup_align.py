...
def align(X, Y, R, lr=10., bsz=200, nepoch=5, niter=500,
    ...
    for epoch in range(1, nepoch + 1):
        for _it in range(1, niter + 1):
            ...
            P = ot.sinkhorn(np.ones(bsz), np.ones(bsz), C, reg, stopThr=1e-2)
            # compute gradient
...
def convex_init(X, Y, niter=100, reg=0.05, apply_sqrt=False):
    ...
    for it in range(1, niter + 1):
        ...
        q = ot.sinkhorn(np.ones(n), np.ones(n), G, reg, stopThr=1e-2)
...
t0 = time.time()
R = align(x_src, x_tgt, R0.copy(), bsz=args.bsz, lr=args.lr, niter=250 ,
